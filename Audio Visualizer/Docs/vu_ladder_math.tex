...
I adjust both the top and bottom resistor per stage: two free values hit
each ratio with less error, and it gives me the most flexibility to pick
parts JLCPCB stocks in its \emph{basic} catalog (extended parts each add a
loading fee). The error column is how far the built threshold lands from the
ideal $V_k$. Worst case is 1.4\%, and adjacent thresholds are 33\% apart, so
turn-on order is never in danger:
